#+TITLE: 自举脚本：把新 Emacs 接到配置仓库
#+AUTHOR: Maeiee
#+PROPERTY: header-args:shell :results silent

* 为什么把自举脚本也写进 Org

配置仓库要能解释自己，也要能重建自己。但这里有一个“先有鸡还是先有蛋”的问题：
尚未接入配置的 Emacs，无法先执行 Org tangle。因此仓库同时提交：

- =bootstrap.org=：可阅读、可维护的唯一源码；
- =bootstrap.sh=：由本文件生成、可在空白机器直接执行的产物。

这也是仓库中唯一建议提交的生成文件。修改下面的源码块后，运行
=make bootstrap=，然后把两个文件一起提交。

* 脚本

#+begin_src shell :tangle bootstrap.sh :shebang "#!/usr/bin/env bash" :tangle-mode (identity #o755)
set -Eeuo pipefail
