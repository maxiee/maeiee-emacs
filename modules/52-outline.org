...
=imenu-list 20210420= 的鼠标回调还把参数当作完整鼠标事件，但 Emacs 的
文本按钮实际传入指向按钮的 marker。当正文窗口仍被选中时，旧回调会把
正文误当成点击来源并放弃动作；当 =*Ilist*= 恰好被选中时又会成功，
因而表现为不稳定的单击。下面的兼容层直接从 marker 获取缓冲区和位置，
不再依赖当前窗口焦点。
...
2. 先点击正文，再连续单击 Outline 中的不同叶子标题，确认每次都跳转。
3. 在 Outline 中用 =j/k= 移动，按 =RET= 跳到正文，按 =l= 预览但保留焦点。
4. 在正文新增或移动一个标题，短暂停顿，确认右侧列表自动更新。
5. 保持焦点位于 Outline 并等待一次自动刷新，确认目录不会一闪而过后清空。
6. 按 =q= 或再次按 =SPC w o=，确认 Outline 关闭。
7. 切到不提供 Imenu 索引的普通缓冲区，确认侧边栏不会保留上一文件的大纲。
