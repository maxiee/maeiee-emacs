...
- 本章只负责平台映射；新建缓冲区、打开配置和项目查找等通用动作定义在
  =01-core.org=，这里仅为它们绑定 Command 键入口。
...
这些绑定只增加 macOS 用户熟悉的入口，不替换 Emacs 原生键位。绑定目标尽量使用
内置命令或 =01-core.org= 中的通用 Maeiee 命令，使平台层不承载业务逻辑。
...
3. 用 ⌘N、⌘P 和 ⌘, 调用核心层命令，再用对应的 =M-x= 命令验证结果一致。
4. 保留 =C-x C-s=；它不是“旧键位垃圾”，而是 Emacs 生态的共同语言。
